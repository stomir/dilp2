...
def main(aspfile: str, problem : str,  args : Dict[str,int],
         target : str, bk : List[str], models : int = 100,
         postfix: str = "", counter : int = 1):
    sep= "_" if postfix != "" else ""
    ctrl = clingo.Control([str(models)])
    with open(problem+sep+postfix+"/"+problem+".lp", 'r') as file, open(aspfile, 'r') as aux:
        program = ''.join(list(aux))+"\n"+''.join(list(file))
        ...
        example_loc = os.path.join(current, problem+sep+postfix+"/"+problem+"/")
        ...
        with ctrl.solve(yield_=True) as res: #type: ignore #I don't actually know, possibly type hints for clingo are incorrect?
            for m in res:
                ...
                if not par or not gpar or not agpar: continue
                par[-1] = lastnl(par)
                gpar[-1] = lastnl(gpar)
